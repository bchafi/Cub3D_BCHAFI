...
int main_work(t_unit *p)
{
    ...
    while (x < WIDTH)
    {
        ...
        if (rayDirX < 0) { stepX = -1; sideDistX = (p->pos_x - mapX) * deltaDistX; }
        else { stepX = 1; sideDistX = (mapX + 1.0 - p->pos_x) * deltaDistX; }
        if (rayDirY < 0) { stepY = -1; sideDistY = (p->pos_y - mapY) * deltaDistY; }
        else { stepY = 1; sideDistY = (mapY + 1.0 - p->pos_y) * deltaDistY; }

        // 3. DDA (Same as before)
        hit = 0;
        while (hit == 0)
        {
            if (sideDistX < sideDistY) { sideDistX += deltaDistX; mapX += stepX; side = 0; }
            else { sideDistY += deltaDistY; mapY += stepY; side = 1; }
            if (p->map[mapY][mapX] == '1') hit = 1;
        }
    ...
    printf("Main work function called for rendering.\n");
    return (0);
}
...
void    raycast_engine(t_var *vars)
{
    ...
    // Raycasting logic will be implemented here
    if (init_mlx(&player))
    {
