...
/*   Updated: 2026/01/11 18:14:15 by bchafi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	close_window(void)
{
	...
	mlx_save = data->mlx;
	if (data->img)
		mlx_destroy_image(mlx_save, data->img);
	if (data->mlx_window)
		mlx_destroy_window(mlx_save, data->mlx_window);
	ft_free(NULL, FREE_ALL);
	if (mlx_save)
	{
		mlx_destroy_display(mlx_save);
		free(mlx_save);
	}
	exit(0);
	return (0);
...
{
	1 && (data->key_a = 0, data->key_d = 0,
		data->key_s = 0, data->key_w = 0,
		data->key_left = 0, data->key_right = 0);
	...
	data->ray_tip_x = data->player_posx
		+ 80 * cos(data->angle);
	data->ray_tip_y = data->player_posy
		+ 80 * sin(data->angle);
